...
ro_opt_model = LpProblem('Recruiting_Office_Placement_&_Sizing')
...
ro_opt_model += lpSum(delta[i] for i in roffices)
# Constraints
for i in roffices:
...
# New stuff - sum up all demand received/felt over all Mkts5
tmp_sum = 0
for j in markets:    
    if j in IR_ros.keys():   # if this mkt is in IR of an RO
        tmp_sum += lpSum(attr[i]*x[i][j] for i in IR_ros[j])
...
ro_opt_model += tmp_sum >= 2500#5592, 'meet mkt demand'
